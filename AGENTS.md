...
**Tool Cards overlay is a mutable view bolted onto an append-only transcript.** The main transcript prints through Ink's `<Static>`, which is permanent once flushed — a historical tool card can't be redrawn wider or taller after the fact, so the transcript caps every card body to a fixed line count and shows "…omitted" past it. `Ctrl+O` or `/tools` (`src/tui/toolCards/ToolCardsOverlay.tsx`) opens a separate, non-`<Static>` panel over the same `tool/call`/`tool/result` events, where the selected card renders through `formatToolCardDetail` (uncapped) instead of the transcript's capped `formatEvent` path, and can be scrolled (`↑↓`/`PageUp`/`PageDown`/`Home`/`End`) once expanded (`Enter`/`Space`). The overlay's own height is capped to a fixed row count rather than the terminal's full height, since there's no alternate-screen buffer here and a near-full-height live frame would force a real terminal scroll Ink can't undo when the overlay later shrinks.

**`/trajectory` ledger rows carry a kind tag, mirroring the web portal.** `TrajectoryLedger.tsx`/`layout.ts` color-tag each row USER/CONTEXT/ASSISTANT/TOOL/HEADER, splitting the former single `'user'` record kind into `'user'` (a direct human prompt) and `'context'` (plugin-injected AGENTS.md, skill content, cron notices, …) by the underlying event's `source.kind` — the same split `formatEvent` already makes for transcript rendering. Only a `'user'` row's full text reaches the ledger's Payload tab; a `'context'` row stays collapsed there too, so nothing leaks the full injected-context payload into the inspector. An assistant row with no visible text but non-empty reasoning shows a reasoning preview instead of falling straight to "(tool calls only)".
