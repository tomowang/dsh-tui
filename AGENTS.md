...
**Tool Cards overlay still caps card bodies, even though the transcript is no longer append-only-forever.** The transcript's `formatEvent` path still caps every tool card's body to a fixed line count with "…omitted" past it (`MAX_CARD_LINES` in `src/render.ts`) — a card scrolled out of `ScrollView`'s viewport is still just sitting in `documentContainer`, cheap to keep uncapped in principle, but the transcript is meant to read as a log, not a set of expandable panels. `Ctrl+O` or `/tools` (`src/tui/toolCards/ToolCardsOverlay.ts`) opens a separate overlay over the same `tool/call`/`tool/result` events, where the selected card renders through `formatToolCardDetail` (uncapped) instead of `formatEvent`'s capped path, and can be scrolled (`↑↓`/`PageUp`/`PageDown`/`Home`/`End`) once expanded (`Enter`/`Space`) — its own hand-rolled scroll-offset math (not pi-tui's `ScrollView`, since a `showOverlay` panel isn't part of the constrained `VStack`/`ScrollView` layout tree) reads the live terminal height off `tui.terminal.rows` each render so it adapts to a resize.
