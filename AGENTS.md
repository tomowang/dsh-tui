...
Early scaffold: settled session events print to native scrollback; a live status bar and boxed prompt sit pinned to the terminal's bottom row. Assistant text now streams live from `assistant/chunk` (folded via `@deepseek-ai/dsh-llm`'s `BlockAssembler` in `TuiStore`, rendered in the live region below `<Static>` until the step settles — see `src/tui/store.ts`, `src/tui/StreamingLine.tsx`). `tool/call`/`tool/result` events now render through a tool's declared `presentCall`/`presentResult` (`@deepseek-ai/dsh-tools`) when available — a diff, a terminal block, grouped search matches, a line-numbered read, web citations — falling back to the original flat line otherwise (see `formatEvent` in `src/render.ts`); unlike the removed first-party TUI's `Ctrl+O` fold, there's no expand/collapse (Ink's `<Static>` prints are permanent), so a card body is capped to a fixed line count instead. Approvals and model questions (`ask_user_question`, plan-mode review) are now answered in-terminal — see the "In-terminal approval/question answerers" architecture note above. Not yet implemented (see README for details): full-screen differential rendering with terminal restoration on failure. The removed first-party `@deepseek-ai/dsh-tui` (deleted from the harness repo, recoverable from git history) remains the reference for that direction.
